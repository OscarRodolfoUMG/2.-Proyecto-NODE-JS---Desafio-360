--Base de datos modificada para la fase de Node.js - Desafio 360
...
AS
BEGIN
    SELECT id_cliente, nombre_comercial, razon_social, direccion, telefono, fk_id_usuario
    FROM Clientes;
END;
GO
...
BEGIN
    BEGIN TRANSACTION;
    BEGIN TRY
        INSERT INTO Clientes (nombre_comercial, razon_social, direccion, telefono, fk_id_usuario)
        VALUES (@nombre_comercial, @razon_social, @direccion, @telefono, @fk_id_usuario);
        COMMIT TRANSACTION;
    END TRY
    BEGIN CATCH
        ROLLBACK TRANSACTION;
